...
def load_csv(csv_file_path,table_name):
    ...
    # print(b)
    # ['"database_name"', '"table_name"', '"index_name"', '"last_update"', '"stat_name"', '"stat_value"', '"sample_size"', '"stat_description"\n']
    # 生成的list，自动给元素增加了''，所以拼接成的create sql 语句执行报错。create table xx("a" varchar(20));对于 "a" 格式报错。
    # 对list循环，拼接数据建表语句。
    colum = ''
    for a in b:
    ...
    conn.close()
    cur.close()
load_csv("E:\\study\\python\\python读取文件\\innodb_index_stats.csv","my_database_stats")
